...
def get_kaguya_xy(root_folder):
    """
    主函數，掃描指定資料夾，解析 .alp, .anm, 和 .prs 檔案並產生座標文字檔。
    """
    print("\nScanning for .alp, .anm, and .prs files...")
    all_files = pathlib.Path(root_folder).rglob("*")
    # 將 .prs 加入掃描目標
    target_files = [f for f in all_files if f.is_file() and f.suffix.lower() in ['.alp', '.anm', '.prs']]

    if not target_files:
        print("No .alp, .anm, or .prs files found in the specified path.")
        return
    ...
    for file_path in target_files:
        ...
        try:
            ...
            # 新增 .prs 檔案的處理邏輯
            elif file_path.suffix.lower() == '.prs':
                human, auto = process_prs_file(file_path, root_folder)
            else:
                continue
...
def process_prs_file(file_path, root_path):
    """
    處理單一 .prs 檔案 (基於 ImageAO.cs 的結構)。
    """
    with open(file_path, "rb") as f:
        # 檢查檔案標頭是否為 'AO'
        signature = f.read(2)
        if signature != b'AO':
            return [], [] # 如果不是 'AO' 開頭，則跳過

        # 根據 ImageAO.cs 的結構，跳到 OffsetX 的位置
        # 0x02: Width (4 bytes)
        # 0x06: Height (4 bytes)
        # 0x0A: BPP (2 bytes)
        # 0x0C: OffsetX (4 bytes) <-- 我們要讀取這個
        # 0x10: OffsetY (4 bytes) <-- 還有這個
        f.seek(12) 
        pos_x = int.from_bytes(f.read(4), byteorder="little", signed=True)
        pos_y = int.from_bytes(f.read(4), byteorder="little", signed=True)
