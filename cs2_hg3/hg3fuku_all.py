...
# --- 部件分組函式 ---
def group_files_by_prefix(file_paths, default_group_name="default"):
    groups = {}
    for path in file_paths:
        filename = os.path.basename(path)
        prefix = default_group_name
        if '_' in filename: prefix = filename.split('_', 1)[0]
...
def process_task(args):
    ...
    for part_path in parts_list: # 列表在傳入前已經排好序
...
def process_character(char_dir, coords_data):
    ...
    kuchi_cycle = kuchi_files or [None]
    hoho_cycle = hoho_files or [None]
    effect_cycle = effect_files or [None]
    
    for fuku_item, kao, kami, kuchi, hoho, effect in itertools.product(
        fuku_items, kao_files, kami_cycle, kuchi_cycle, hoho_cycle, effect_cycle):
