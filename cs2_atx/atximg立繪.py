...
def fuku_sort_key(filepath):
    short_name = get_short_name(filepath)
    key1 = len(short_name)
    transformed_name = ""
    for char in short_name:
        if char.isalpha(): transformed_name += '0' + char
        elif char.isdigit(): transformed_name += '1' + char
        else: transformed_name += '2' + char
    key2 = transformed_name
    return (key1, key2)
...
def process_character_directory(char_dir):
    ...
    for fuku_base_name, fuku_group_files in fuku_groups:
        ...
        fuku_group_files.sort(key=fuku_sort_key)
        print(f"  > Fuku 部件合成順序: {[os.path.basename(p) for p in fuku_group_files]}")
        ...
        if kami_files:
            ...
            body_name = f"{char_name}_{fuku_base_name}"; body_bases[body_name] = fuku_base_img
        
        print(f"--- [{fuku_base_name}] 流程 A: 建立標準臉 (fuku_face) ---")
            ...
            for kao_path in kao_files:
                ...
                if kuchi_files:
                    for kuchi_path in kuchi_files:
                        ...
                        final_img = Image.fromarray(final_np); final_name = "_".join(final_name_parts) + ".png"; output_path = os.path.join(fuku_face_dir, final_name)
                        if not os.path.exists(output_path): final_img.save(output_path)
                        face_images[final_name] = final_img
                else:
                    final_img = Image.fromarray(base_np); final_name = "_".join(name_parts) + ".png"; output_path = os.path.join(fuku_face_dir, final_name)
                    if not os.path.exists(output_path): final_img.save(output_path)
                    face_images[final_name] = final_img
        ...
        if effect_files:
            ...
            for face_name, face_img in face_images.items():
                for effect_path in effect_files:
                    ...
                    final_name = f"{os.path.splitext(face_name)[0]}_{get_short_name(effect_path)}.png"; output_path = os.path.join(fuku_face_effect_dir, final_name)
                    ...
                    for kao_path in kao_files:
                        ...
                        if kuchi_files:
                            for kuchi_path in kuchi_files:
                                 ...
                                 final_name = "_".join(name_parts + [get_short_name(kuchi_path)]) + ".png"; final_img = Image.fromarray(final_np); output_path = os.path.join(fuku_face_hoho_dir, final_name)
                                 if not os.path.exists(output_path): final_img.save(output_path)
                                 hoho_face_images[final_name] = final_img
                        else:
                            final_img = Image.fromarray(kao_np); final_name = "_".join(name_parts) + ".png"; output_path = os.path.join(fuku_face_hoho_dir, final_name)
                            if not os.path.exists(output_path): final_img.save(output_path)
                            hoho_face_images[final_name] = final_img
            ...
            if effect_files:
                ...
                for face_name, face_img in hoho_face_images.items():
                    for effect_path in effect_files:
                        pos = get_image_position(effect_path, offset_data); new_pos = (pos[0]+global_offset[0], pos[1]+global_offset[1])
                        with Image.open(effect_path) as p_img: final_np = composite_numpy(np.array(face_img), p_img.convert("RGBA"), new_pos)
                        final_name = f"{os.path.splitext(face_name)[0]}_{get_short_name(effect_path)}.png"; output_path = os.path.join(fuku_face_hoho_effect_dir, final_name)
                        if not os.path.exists(output_path): Image.fromarray(final_np).save(output_path)

def main():
    script_dir = os.getcwd(); print(f"開始掃描目標資料夾於: {script_dir}")
    char_dirs_to_process = [root for root, dirs, files in os.walk(script_dir) if FUKU_DIR_NAME in dirs and OFFSET_FILE_NAME in files and 'output' not in root[len(script_dir):]]
