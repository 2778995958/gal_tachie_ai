...
def find_best_x_offset_by_left_edge(torso_img, legs_img, crop_height=1):
    ...
    if torso_left_x == -1 or legs_left_x == -1:
        print("  -> 警告：在真實接合處找不到有效輪廓，無法對齊。")
        return 0
...
def bake_template(large_image_path, small_images_folder):
    # (此函式與 v22 完全相同)
    print("\n--- 進入「烘焙模式」---")
    # ... (程式碼省略以保持簡潔) ...
    small_folder_name = os.path.basename(os.path.normpath(small_images_folder))
    json_path = f"{small_folder_name}.json"
    default_bake_params = {"scale_factor": 0.5, "x_offset": 0, "y_offset": 0}
    params = {}
    if not os.path.exists(json_path):
        print(f"未找到設定檔，將為您建立一個包含預設烘焙參數的 '{json_path}'。")
        print("您可以修改此檔案中的 bake_params，然後重新執行烘焙。")
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump({"bake_params": default_bake_params, "apply_params": {}}, f, indent=4, ensure_ascii=False)
        params = default_bake_params
    else:
        try:
            with open(json_path, 'r', encoding='utf-8') as f: params = json.load(f)["bake_params"]
            print(f"偵測到現有設定檔 '{json_path}'，將使用其中的參數進行烘焙。")
        except Exception:
            print(f"設定檔 '{json_path}' 格式不對，將使用預設參數。")
            params = default_bake_params
    try:
        ...
        base_img = Image.open(os.path.join(small_images_folder, first_small_file)).convert('RGBA')
        ...
        bake_time_width = base_img.width
        if base_img.height > 1: base_img = base_img.crop((0, 0, base_img.width, base_img.height - 1))
        ...
        json_config = {"bake_params": params, "apply_params": {"legs_template_file": legs_template_path, "bake_time_width": bake_time_width}}
...
if __name__ == "__main__":
    ...
    if '--bake' in args:
        try:
            ...
            bake_template(large_img_path, small_folder_path)
