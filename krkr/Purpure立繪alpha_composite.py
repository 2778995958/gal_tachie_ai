# 【最終典藏版 Ver. 33.1 - Alpha Composite 修正版】
import pandas as pd
...
def load_layer_data_with_paths(filepath):
    ...
    try:
        ...
        print("[INFO] 完整路徑建立完成。")
        # print(df['full_path'].to_list()) # 減少 log 輸出
        return df
...
# --- 2. 影像合成核心邏輯 (已修改為 Alpha Composite) ---
def create_composite_image_relative(layers_to_draw, base_info, output_path, log_file):
    ...
    try:
        ...
        for part_info in layers_to_draw:
            ...
            # --- [修改處] 使用 Alpha Composite 進行合成 ---
            # 1. 建立一個與主畫布同樣大小的透明圖層
            layer_canvas = Image.new("RGBA", (canvas_width, canvas_height), (0, 0, 0, 0))
            
            # 2. 將部件貼到這個暫存圖層的正確位置 (這裡仍用 paste，因為是貼到空圖層上)
            layer_canvas.paste(part_img, (paste_x, paste_y))
            
            # 3. 使用 alpha_composite 將暫存圖層疊加到主畫布 (這是關鍵步驟)
