# 【最終典藏版 Ver. 33.3 - 嚴格分層順序版 (Dress->Face->Top)】
import pandas as pd
...
# ==============================================================================
# --- 使用者設定區 ---
SPECIAL_UNDERLAY_IDS = {83020, 83021} 
PRIORITY_OVERLAY_PATHS = {'かぶせ 水着0'}
# ==============================================================================
...
def process_character(txt_path, sinfo_path, log_file):
    ...
    for dress_name, diffs in dress_rules.items():
        for diff_id, dress_paths in diffs.items():
            ...
            # --- 【關鍵】辨識 Dress 的分層 ---
            # last_dress_path: 清單中最後一個定義的服裝部件 (例如: 鬼面)
            # forced_top_layer_id: 對應的 ID，將被強制移到最上層
            ...
            # 傳遞參數給 process_face_combinations
            process_face_combinations(final_face_rules, dress_layer_ids, forced_top_layer_id, path_to_id, id_to_path, layer_df, char_base_name, current_dress_key, generated_filenames_set, log_file, output_folder)
            
            for face_name, conditions in conditional_faces.items():
                if matches_condition(dress_name, condition):
                    process_face_combinations({face_name: face_paths}, dress_layer_ids, forced_top_layer_id, path_to_id, id_to_path, layer_df, char_base_name, f"{current_dress_key}@{condition}", generated_filenames_set, log_file, output_folder)
...
# --- 【關鍵修改】嚴格的分層合成函數 ---
def process_face_combinations(face_rule_dict, dress_layer_ids, forced_top_layer_id, path_to_id, id_to_path, layer_df, char_base_name, dress_info_str, generated_filenames_set, log_file, output_folder):
    
    # dress_layer_ids: 所有服裝部件 ID (包含最後一件)
    # forced_top_layer_id: 最後一件服裝部件的 ID (需在最上層)
    ...
    for face_name, face_paths in face_rule_dict.items():
        ...
        if not all_paths_found: 
            print(f"  ⏭️  跳過組合: {face_name} (Sinfo 路徑錯誤)")
            continue
        ...
        if missing_parts_log:
            log_file.write(f"[組合跳過] {combination_context}: 缺檔: {', '.join(missing_parts_log)}\n")
            print(f"  ⏭️  跳過組合: {face_name} (缺檔)")
            continue
        ...
        # --- 【核心邏輯升級】嚴格分層排序 ---
        # 1. 分離出三組圖層
        #    Group A: 一般服裝 (Dress, 排除最後一件)
        #    Group B: 表情 (Face)
        #    Group C: 置頂服裝 (Top Dress / Mask)
        ...
        for item in layers_data:
            ...
            if lid == forced_top_layer_id:
                group_top.append(item)
            elif lid in face_id_set:
                group_face.append(item)
            elif lid in dress_id_set:
                group_dress.append(item)
            else:
                # 備用：萬一有漏網之魚，預設歸類到 dress
                group_dress.append(item)

        # 2. 各組內部依照 top 座標排序 (確保組內正確堆疊)
        group_dress.sort(key=lambda x: x['top'])
        group_face.sort(key=lambda x: x['top'])
        # group_top 通常只有一件，不需排序，若有多件也依照 top 排序
        group_top.sort(key=lambda x: x['top'])

        # 3. 嚴格合併：Dress -> Face -> Top
        layers_to_draw = group_dress + group_face + group_top
        
        # 4. 尋找座標基準 (通常是 group_dress 的第一件，即身體)
        if group_dress:
            base_info_for_coords = group_dress[0]
        elif layers_to_draw:
        ...
        dress_part_ids = [lid for lid in final_used_ids if lid in dress_id_set] # 包含 mask
        other_part_ids = [lid for lid in final_used_ids if lid not in dress_id_set] # face parts
        ...
        if create_composite_image_relative(layers_to_draw, base_info_for_coords, output_path, log_file):
            ...
            log_file.write(f"[成功生成] {combination_context}: '{output_filename}'\n")
