...
PRIORITY_OVERLAY_PATHS = {'かぶせ 水着00'}
MAX_WORKERS = os.cpu_count() or 4  # 多線程數量，預設為 CPU 核心數
# ==============================================================================
    ...
    try:
        ...
        for layer_id in df['layer_id']:
            ...
            while current_id in id_to_info and current_id not in visited_ids:
                ...
                if info['name']:
                    normalized_name = normalize_path_string(info['name'])
        ...
        df['full_path'] = df['full_path'].fillna(df['name'].apply(normalize_path_string))
        print("[INFO] 完整路徑建立完成。")
        return df
...
def process_character(txt_path, sinfo_path, log_file):
    ...
    print(f"\n[INFO] 共收集到 {total_tasks} 個待合成任務，使用 {MAX_WORKERS} 個線程開始處理...")
    success_count = 0
    log_lock = threading.Lock()

    def composite_task(task_info):
        layers_info, out_path, context, filename = task_info
        result = create_composite_image_relative(layers_info, out_path, log_file)
        return result, context, filename

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(composite_task, task): task for task in pending_tasks}
        for future in as_completed(futures):
            result, context, filename = future.result()
            if result:
                success_count += 1
                with log_lock:
                    log_file.write(f"[成功生成] {context}: 已儲存為 '{filename}'\n")
