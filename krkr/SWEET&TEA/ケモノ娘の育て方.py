# 【Purpure立繪合成系統 Ver. 45.0 - 智能五官過濾版】
# 修正重點：
# 1. 修正「全表情疊加」BUG：
#    增加自動配件的「黑名單前綴」(目, 口, 眉, 頬, パーツ)。
#    現在只有「耳朵、尾巴」等非五官部件會被自動加入，五官必須由表情別名 (Alias) 控制。
# 2. 保持所有先前功能 (三明治疊圖、Eye鎖定、Dummy忽略)。
...
GENERATE_SIMPLE_MODE = False     # 是否生成無 mimi 版
OUTPUT_ROOT = "output"           # 輸出資料夾
TARGET_EYE_VAL = 2               # 強制鎖定 eye==2

# --- 自動配件排除清單 ---
# 如果 fgname 以這些字開頭，絕對不會被當作自動配件 (必須透過 fgalias 呼叫)
EXCLUDED_AUTO_PREFIXES = ["目_", "口_", "眉_", "頬_", "パーツ_"]
# ==============================================================================
...
class LayerSystem:
    ...
    def get_layer_by_path(self, path):
        if not path or path.lower() == "dummy": return None
                    ...
                    if cmd == 'dress':
                        if len(parts) >= 5:
                            ...
                            if d_name not in self.dress_layers:
                                ...
                                self.dress_order.append(d_name)
...
def check_condition(condition_str, mimi_val):
    ...
    cond = condition_str.replace(" ", "")
    if "mimi" in cond:
        if f"mimi=={mimi_val}" not in cond: return False
    if "eye" in cond:
        if f"eye=={TARGET_EYE_VAL}" not in cond: return False
    return True
...
def process_character(txt_file, sinfo_file, log_file):
    ...
    for dress_name in rule_sys.dress_order:
        ...
        # 三明治 - Base & Top
        base_layer_info = None
        top_layer_infos = []
        
        if target_layers:
            base_l = layer_sys.get_layer_by_path(target_layers[0])
            if base_l: base_layer_info = base_l
            
            for path in target_layers[1:]:
                l = layer_sys.get_layer_by_path(path)
                if l: top_layer_infos.append(l)

        if not base_layer_info and not top_layer_infos:
            print(f"[跳過] 服裝 {dress_name} 無法讀取")
            continue
        ...
        for mimi_val in loop_variants:
            ...
            if not is_simple_mode:
                ...
                for logic_name, variants in rule_sys.fgnames.items():
                    # 判斷是否為潛在的自動配件 (專用 或 通用)
                    is_candidate = logic_name.endswith(suffix) or ("@" not in logic_name)
                    
                    if is_candidate:
                        # 【關鍵修正】檢查是否為排除的五官前綴
                        # 如果是 "口_", "目_" 等開頭，強制排除，不讓它變成自動配件
                        is_excluded = False
                        for prefix in EXCLUDED_AUTO_PREFIXES:
                            if logic_name.startswith(prefix):
                                is_excluded = True
                                break
                        
                        if not is_excluded:
                            # 只有通過黑名單檢查的部件 (如 耳、尾)，才檢查條件並加入
                            for v in variants:
            ...
            for alias_name, components in rule_sys.aliases.items():
                ...
                # A. Base
                if base_layer_info: draw_queue.append(base_layer_info)
                ...
                for comp_key in components:
                    ...
                    for cand in candidates:
                         if is_simple_mode:
                             if "eye" in (cand['condition'] or ""):
                                 if check_condition(cand['condition'], TARGET_EYE_VAL):
                ...
                # D. Top
                draw_queue.extend(top_layer_infos)
...
if __name__ == '__main__':
    ...
    print(f"程式啟動 (Ver 45.0 - Smart Auto-Parts Filter)... Log: {LOG_FILENAME}")
